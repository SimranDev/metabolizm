...
- `pnpm test` — vitest (root [vitest.config.ts](vitest.config.ts), `apps/api/src/**/*.test.ts`). **Deliberately one suite only**: [groups/masking.test.ts](apps/api/src/groups/masking.test.ts), the regression guard for the masking invariant below. This is not an invitation to add general unit tests — the repo is otherwise runner-free by choice. Tests are excluded from `tsconfig.build.json` so they never reach `dist/` or the image.
