...
| `pnpm clean` | The counterpart to `pnpm install` — remove build outputs and caches across every package (see "Cleaning" below) |
| `docker build -t metabolizm-api .` | Build the production API image |

### Cleaning (`pnpm clean`)

`pnpm install` populates every workspace package; `pnpm clean` ([scripts/clean.mjs](../scripts/clean.mjs)) empties them again. It discovers packages via `pnpm list -r`, so a new one under `apps/`/`packages/` is covered without editing the script, and everything it removes is gitignored and regenerated by a build, an install or a dev server.

```bash
pnpm clean                # dist/, .expo/cache, node_modules/.cache, *.tsbuildinfo, Metro's $TMPDIR caches
pnpm clean --dry-run      # print what would go, delete nothing (works with every flag below)
pnpm clean --gradle       # + android's build state (.gradle, build/, app/.cxx) — keeps the project
pnpm clean --native       # + apps/mobile/ios and apps/mobile/android outright (implies --gradle)
pnpm clean --deps         # + every node_modules
pnpm clean --store        # + pnpm store prune
pnpm clean --all          # everything, incl. expo's generated type declarations
```

Bare `pnpm clean` is the safe default: it keeps `node_modules` and the native projects, so the recovery is just `pnpm api` / `pnpm typecheck` (both rebuild shared + db). Each escalation costs real time — `--deps` needs a `pnpm install`, `--native` a full Gradle/Xcode rebuild — so reach for them only when a stale artifact is actually suspected.

Three things are worth knowing about what it targets:

- **Metro's cache is not in the repo.** It lives in `$TMPDIR` (`metro-*`, `haste-map-*`, `react-native-packager-cache-*`), which is why deleting `node_modules` alone can leave a stale bundle and why `expo start --clear` exists. `pnpm clean` removes those too; for a stale bundle *during* a session, `pnpm start:inspect` (or `npx expo start --clear`) is the faster fix — no need to clean the whole workspace.
- **`.expo/types` is held back from the default clean.** `router.d.ts` is the `typedRoutes` declaration and sits in mobile's tsconfig `include`. Without it `tsc --noEmit` still *passes* — it just stops checking route strings — so removing it by default would quietly weaken `pnpm typecheck` instead of failing loudly. Only `--all` takes it; Metro regenerates it on the next `pnpm start`.
- **`--native` deletes `apps/mobile/ios` and `apps/mobile/android` outright**, which is safe precisely because neither is checked in — CNG/prebuild regenerates both from [app.json](../apps/mobile/app.json) on the next `pnpm ios` / `pnpm android`. If you have hand-edited native files there, they are already outside version control and this will take them with it.

The script refuses to remove anything outside the repo root or `$TMPDIR`, and reports the space reclaimed.

#### Gradle, and what is deliberately out of reach

Android build state is by far the largest thing the workspace accumulates — `apps/mobile/android/app/.cxx` (the NDK/CMake output) is **~167 MB on its own**, against ~6 MB for every JS-side cache combined. `--gradle` is its own tier for that reason: it clears `.gradle`, `build/`, `.kotlin`, `app/build` and `app/.cxx` while **keeping the generated project**, so recovery is one `pnpm android` rather than a prebuild plus a full regenerate. It is the equivalent of `./gradlew clean` and the right first move when a native build fails in a way the source doesn't explain. `--native` implies it.

**`~/.gradle` is never touched, by any flag** — including `--all`. It holds Gradle's dependency cache (300 MB+) and is shared with every other Android project on the machine, so deleting it re-downloads AGP and the entire dependency graph for all of them. The same reasoning keeps Xcode's `DerivedData` out of scope; only the project-local `apps/mobile/ios/build` is cleared. Both are machine-wide state that a repo script has no business owning — clear them by hand if you actually mean to:

```bash
rm -rf ~/.gradle/caches                              # forces a full Gradle re-download
rm -rf ~/Library/Developer/Xcode/DerivedData         # forces a full Xcode rebuild
```

`pnpm clean --store` is the one exception, and only because `pnpm store prune` is *not* destructive in that way — it removes packages nothing references, so the cost is bounded.
